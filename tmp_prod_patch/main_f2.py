...
# 🎯 FOTO TARGET
TARGET_PHOTO_INDEX = int(os.getenv("TARGET_PHOTO_INDEX", "1"))
...
LOCAL_BAN_GUARD = os.getenv("LOCAL_BAN_GUARD", "0").strip() == "1"

# Hashes SHA256 (bytes de imagen decodificada) de placeholders conocidos.
DEFAULT_PLACEHOLDER_PHOTO_HASHES = (
    "96b2e4b82a6a4c2bac03beb3bc86d938f5f41c40281f6aec8ac5d77e6b5ba93c,"
    "c8d36c8c2d7a9e80b4529125b15d96a8483fe5b9cc5ac573557a6931800c997c"
)
_placeholder_hashes_raw = os.getenv(
    "PLACEHOLDER_PHOTO_HASHES", DEFAULT_PLACEHOLDER_PHOTO_HASHES
)
PLACEHOLDER_PHOTO_HASHES = {
    h.strip().lower()
    for h in _placeholder_hashes_raw.split(",")
...
RE_WAIT_SECS_5 = re.compile(
    r"ANTI\s*[-–—]?\s*SPAM[\s:\.\-]*ESPERA\s*([\d\.]+)\s*(?:s|segundos?)",
    re.IGNORECASE,
)
...
def parse_antispam_wait_seconds(text: str) -> Optional[float]:
    ...
    for rx in (RE_WAIT_SECS_1, RE_WAIT_SECS_2, RE_WAIT_SECS_3, RE_WAIT_SECS_4, RE_WAIT_SECS_5):
        m = rx.search(text or "")
        if not m:
...
def photo_sha256_from_b64(photo_b64: Optional[str]) -> Optional[str]:
    if not photo_b64:
        return None
    try:
...
def is_placeholder_photo_b64(photo_b64: Optional[str]) -> bool:
    h = photo_sha256_from_b64(photo_b64)
    return bool(h and h in PLACEHOLDER_PHOTO_HASHES)
...
UPSERT_COLS = [
    ...
    "fecha_inscripcion", "fecha_emision", "fecha_caducidad",
    "padre",
...
async def fetch_payload_from_bot(
    ...
    async with client.conversation(chat, timeout=None) as conv:
        ...
        payload_photo_count = 0
        payload_valid_photo_count = 0
        ...
        while True:
            ...
            if text:
                ...
                if wait_s is None and RE_ANTI_SPAM_ON.search(text):
                    wait_s = 15.0
                if wait_s is None and "anti-spam" in text.lower():
                    logger.warning(
                        "[%s] TG anti-spam text no parseado: %s",
                        req_id,
                        text[:160].replace("\n", " "),
                    )
            ...
            if getattr(msg, "photo", None) and is_related:
                ...
                # Evita capturar imagen de espera: solo tomamos fotos cuando
                # ya existe payload de texto (o en el mismo mensaje con caption).
                if text_raw is None:
                    logger.info(
                        "[%s] TG DEFER photo #%d (payload text not captured yet)",
                        req_id,
                        relevant_photo_count,
                    )
                    continue

                payload_photo_count += 1
                logger.info(
                    (
                        "[%s] TG PAYLOAD PHOTO #%d (overall #%d) "
                        "target_payload_photo=%d (sid=%s reply_to=%s)"
                    ),
                    req_id,
                    payload_photo_count,
                    relevant_photo_count,
                ...
                candidate_b64 = base64.b64encode(buf.getvalue()).decode("utf-8")

                if is_placeholder_photo_b64(candidate_b64):
                    logger.info(
                        "[%s] TG SKIP payload photo #%d (placeholder hash)",
                        req_id,
                        payload_photo_count,
                    )
                    continue

                payload_valid_photo_count += 1
                if payload_valid_photo_count == TARGET_PHOTO_INDEX:
                    foto_b64 = candidate_b64
                    logger.info(
                        "[%s] TG FOTO payload valid #%d captured (len=%d)",
                        req_id,
                        TARGET_PHOTO_INDEX,
                        len(foto_b64),
                    )
                    ...
                    logger.info(
                        "[%s] TG SKIP payload valid photo #%d (waiting for valid #%d)",
                        req_id,
                        payload_valid_photo_count,
                        TARGET_PHOTO_INDEX,
                    )
...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        "BOT_USER=%s BOT_COMMAND=%s SESSION_FILE=%s TARGET_PHOTO=%d LOCAL_BAN_GUARD=%s PLACEHOLDER_HASHES=%d",
        BOT_USER,
        BOT_COMMAND,
        str(SESSION_FILE),
        TARGET_PHOTO_INDEX,
        LOCAL_BAN_GUARD,
        len(PLACEHOLDER_PHOTO_HASHES),
    )
...
async def health():
    ...
    if (
        LOCAL_BAN_GUARD
        and app.state.banned_until
        and datetime.now() < app.state.banned_until
    ):
        banned = True
    ...
    return {
        ...
        "local_ban_guard": LOCAL_BAN_GUARD,
...
async def consulta(
    ...
    if refresh != 1:
        ...
        if cached:
            ...
            foto_is_placeholder = is_placeholder_photo_b64(cached.get("foto"))
            logger.info(
                "[%s] DB HIT dni=%s foto_null=%s foto_placeholder=%s complete=%s",
                req_id,
                dni,
                foto_is_null,
                foto_is_placeholder,
            ...
            if is_record_complete(cached) and not foto_is_placeholder:
            ...
            if is_record_complete(cached) and foto_is_placeholder:
                logger.info(
                    "[%s] DB PLACEHOLDER FOTO detectada dni=%s -> consultando TG",
                    req_id,
                    dni,
                )
            ...
            if foto_is_null:
                logger.info(
                    "[%s] DB HIT con foto null dni=%s -> consultando TG",
                    req_id,
                    dni,
                )
    ...
    try:
        ...
        if LOCAL_BAN_GUARD:
            ...
            reason = "banned_set_until_midnight"
        else:
            reason = "banned_from_bot_no_local_guard"
            logger.warning(
                "[%s] TG BANNED detected but LOCAL_BAN_GUARD=0, no local lock applied",
                req_id,
            )

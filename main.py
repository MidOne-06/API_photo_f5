...
# ðŸ“ BASE + .ENV
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•ï¿½ï¿½ï¿½â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# Forzar mensajes en inglÃ©s y client_encoding definido (por defecto UTF8).
...
class AdaptiveInterval:
    ...
    def on_antispam(self):
        ...
        logger.info("ADAPTIVE INTERVAL subiÃ³ a %.1fs tras anti-spam", self.current)
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# âœ… DETECCIÃ“N DE RESPUESTAS DEL GRUPO
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

# --- NO INFO ---
RE_KAISEN_NOINFO = re.compile(
    r"(?:\[[^\]]+\].*?)?no se encontro\s+(?:informaci[oÃ³]n|datos)(?:\s+para\s+este\s+dni)?(?:\s*\[[^\]]+\])?",
...
BOT_INTERNAL_ERROR_PATTERNS = [
    "âŒ Error Interno",
    "OcurriÃ³ un fallo",
    "OcurriÃ³ un error al procesar la consulta",
...
ANTI_SPAM_TEXT_OLD = "ðŸš¨ Â¡AtenciÃ³n! Reporta a tu revendedor ðŸš¨"
...
class BotDniInvalidException(Exception):
    """El grupo respondiÃ³ que el DNI es invÃ¡lido."""
    pass
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# ðŸ§  PARSER KAISEN / VYCOCODE
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

SEP_RE = r"(?:\:|âž¾|Â»|â†’)"
GENERIC_HEADER_RE = re.compile(
    r"^\s*\[[^\]]+\]\s*[A-ZÃÃ‰ÃÃ“ÃšÃ‘a-zÃ¡Ã©Ã­Ã³ÃºÃ¼Ãœ0-9\s]+\s*$",
    re.MULTILINE,
)
...
def _parse_creditos_fields(
    ...
    if parts:
        ...
        # Nuevo formato observado: "CREDITOS âž¾ â™¾ï¸ - 8144631204"
...
def grab_line_variants(text: Optional[str], labels: List[str]) -> Optional[str]:
    ...
    def normalize_label(value: str) -> str:
        ...
        if not raw:
            return ""
        # Normaliza acentos y caracteres extraÃ±os para comparar etiquetas
        # sin depender del encoding exacto del payload.
    ...
    # Fallback robusto: compara llaves de etiquetas con normalizaciÃ³n
    # (acentos/emoji/encoding), manteniendo el mismo conjunto de labels permitido.
    normalized_targets = {
        normalize_label(label) for label in labels if normalize_label(label)
    }
    ...
    for line in text.splitlines():
        if not line or not line.strip():
            continue
        parts = re.split(SEP_RE, line, maxsplit=1, flags=re.IGNORECASE)
        if len(parts) < 2:
            continue
...
def parse_kaisen_text(text: str) -> Dict[str, Any]:
    ...
    out["genero"] = grab_line_variants(raw, ["GENERO", "GÃ‰NERO"])

    headers = _header_positions(raw)

    sec_nac = _slice_section(raw, r"NACIMIENTO", headers)
    sec_info = _slice_section(
        raw, r"INFORMACION\s+GENERAL|INFORMACIÃ“N\s+GENERAL", headers
    )
    sec_dir = _slice_section(raw, r"DOMICILIO|DIRECCION|DIRECCIÃ“N", headers)
    sec_ubi = _slice_section(raw, r"UBIGEOS|UBICACION|UBICACIÃ“N", headers)
    ...
    out["grado_instruccion"] = grab_line_variants(
        info_target,
        ["GRADO INSTRUCCION", "GRADO INSTRUCCIÃ“N", "NIVEL EDUCATIVO"],
    )
    ...
    out["fecha_inscripcion"] = _parse_date_ddmmyyyy(
        grab_line_variants(
            info_target, ["FECHA INSCRIPCION", "FECHA INSCRIPCIÃ“N"]
        )
    )
    out["fecha_emision"] = _parse_date_ddmmyyyy(
        grab_line_variants(info_target, ["FECHA EMISION", "FECHA EMISIÃ“N"])
    )
    ...
    out["donante_organos"] = grab_line_variants(
        info_target, ["DONANTE ORGANOS", "DONANTE Ã“RGANOS"]
    )
    ...
    out["restriccion"] = grab_line_variants(
        info_target, ["RESTRICCION", "RESTRICCIÃ“N"]
    )
    ...
    if sec_ubi:
        ...
        out["codigo_postal"] = grab_line_variants(
            sec_ubi, ["CODIGO POSTAL", "CÃ“DIGO POSTAL"]
        )
    ...
    if sec_act:
        ...
        out["acta_defuncion"] = _to_int_from_text(
            grab_line_variants(sec_act, ["DEFUNCION", "DEFUNCIÃ“N"])
        )
    ...
    creditos_line = grab_line_variants(account_target, ["CREDITOS", "CRÃ‰DITOS"])
    if creditos_line is None:
        # Fallback tolerante para variantes con acentos/encoding corrupto:
        # "Créditos", "Cr?ditos", etc.
        m_credit = re.search(
            rf"^[ \t]*CR\S*DIT\S*[ \t]*{SEP_RE}[ \t]*([^\r\n]*)[ \t]*$",
            account_target,
            re.IGNORECASE | re.MULTILINE,
        )
        if m_credit:
            creditos_line = _clean_value(m_credit.group(1))
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# ðŸ—„ï¸ DB Manager (public.clientes)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
class DatabaseManager:
    ...
    def _sanitize_texts_for_latin1(self, record: Dict[str, Any]) -> Dict[str, Any]:
        """
        Si el servidor PG estÃ¡ en LATIN1 y llegan emojis/caracteres fuera de rango,
...
def _validate_bot_entity(entity: Any, source_label: str):
    if isinstance(entity, Channel):
        if getattr(entity, "broadcast", False):
            raise RuntimeError(
                f"{source_label} resolviÃ³ a un CHANNEL broadcast/solo lectura: "
                f"{_entity_summary(entity)}. "
                f"Debes usar el @username del bot (ej. @RelayXGate_bot), no un canal."
            )
        if getattr(entity, "left", False):
            raise RuntimeError(
                f"{source_label} resolviÃ³ a un channel/chat donde la cuenta "
                f"estÃ¡ fuera (left=True): {_entity_summary(entity)}."
            )
...
async def resolve_chat_entity(client: TelegramClient, chat_query: str):
    ...
    if not q:
        raise RuntimeError("BOT_USER vacÃ­o.")
    ...
    if best_match is not None:
        _validate_bot_entity(best_match, f"BÃºsqueda parcial '{q}'")
        return best_match
...
async def _drain_trailing_bot_messages(
    conv: Any,
    req_id: str,
    response_ids: set,
    main_sender_id: Optional[int],
    drain_window_s: float = 1.8,
) -> None:
    """
    Después de capturar foto+texto, drenamos un pequeño buffer de respuestas
    tardías del mismo bot para poder limpiarlas también.
    """
    if not main_sender_id:
        return
    deadline = time.perf_counter() + max(0.0, drain_window_s)
    while time.perf_counter() < deadline:
        timeout_s = min(0.7, max(0.15, deadline - time.perf_counter()))
        try:
            extra = await conv.get_response(timeout=timeout_s)
        except asyncio.TimeoutError:
            break
        extra_id = getattr(extra, "id", None)
        extra_sid = getattr(extra, "sender_id", None)
        if (
            isinstance(extra_id, int)
            and extra_id > 0
            and extra_sid == main_sender_id
        ):
            response_ids.add(extra_id)
            logger.info(
                "[%s] TG DRAIN extra message captured for cleanup id=%s",
                req_id,
                extra_id,
            )
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# ðŸ§­ TELEGRAM: captura FOTO + TEXTO
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
async def fetch_payload_from_bot(
    ...
    async with client.conversation(chat, timeout=None) as conv:
        ...
        while True:
            ...
            # Si ya identificamos al sender principal del bot para esta consulta,
            # marcamos todos sus mensajes para limpieza (incluye fotos sobrantes).
            if (
                isinstance(msg_id, int)
                and msg_id > 0
                and main_sender_id
                and sid == main_sender_id
            ):
                response_message_ids.add(msg_id)
            ...
            # ðŸ” DETECCIÃ“N DE RESPUESTAS ESPECIALES
            # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
            ...
            # --- DNI INVÃLIDO: muere, no guarda BD, no reintenta ---
            ...
            if wait_s is not None:
                # Espera precisa del bot + pequeÃ±o margen configurable.
            ...
            # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
            # âœ… Ã‰XITO: foto + texto completos
            # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
            ...
            if foto_b64 and text_raw:
                ...
                await _drain_trailing_bot_messages(
                    conv,
                    req_id=req_id,
                    response_ids=response_message_ids,
                    main_sender_id=main_sender_id,
                )
                logger.info(
...
async def fetch_payload_with_reconnect(
    ...
    try:
        ...
        # Telethon puede lanzar errores de buffer/clock skew; intentamos un reconnect Ãºnico
        logger.warning(
    ...
    # Reintento Ãºnico tras cualquier error
    try:
...
async def lifespan(app: FastAPI):
    ...
    if isinstance(app.state.bot_chat, User) and not getattr(
        ...
        logger.warning(
            "BOT_USER resolviÃ³ a un usuario normal, no a un bot. "
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# ENDPOINT: /health
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•ï¿½ï¿½ï¿½â•â•â•â•
...
async def consulta(
    ...
    if not dni.isdigit() or len(dni) != 8:
        raise HTTPException(400, "DNI invÃ¡lido.")
    ...
    try:
        ...
        # DNI invÃ¡lido segÃºn el grupo: muere, NO guarda en BD, NO reintenta
